...
logger = logging.getLogger("adapter.olimpbet")

def _n(s: str) -> str:
    return " ".join((s or "").lower().replace(".", " ").replace("-", " ").split())

async def fetch() -> List[Dict[str, Any]]:
    if not config.OLIMPBET_EVENTS_URL:
        return []
    data = await pool.get_json(config.OLIMPBET_EVENTS_URL, timeout=config.OLIMPBET_TIMEOUT)
    if data is None:
        return []
    # Heuristic parsing; adjust to real payload
